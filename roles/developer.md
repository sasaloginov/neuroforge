...
Все артефакты аналитика находятся в `docs/analyst/<shortId>/`:
- `docs/analyst/<shortId>/context.md` — карта затрагиваемого кода. **Читай первым делом**
- `docs/analyst/<shortId>/design/spec.md` — спецификация для реализации
- `docs/analyst/<shortId>/research/` — исследования (по необходимости)

## Процесс работы
1. Прочитай `docs/analyst/<shortId>/context.md` — карта затрагиваемого кода от analyst'а. Используй для навигации, не перечитывай файлы, если информация уже там
2. Прочитай `PROJECT_MAP.md` — общая карта модулей проекта. Используй для понимания зависимостей
3. Прочитай спецификацию `docs/analyst/<shortId>/design/spec.md`
